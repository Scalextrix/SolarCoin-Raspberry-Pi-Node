...
def command_chooser():
    command_choice = input('Issue a command, "help" or "exit" to exit: ').lower()
...
def send():
    ...
    address = input('Send SLR to which address: ')
    amount = float(input('How much SLR to send: '))
...
def unlock_wallet():
    passphrase = getpass.getpass(prompt="What is your SolarCoin Wallet Passphrase: ")
    unlock_time = int(input('unlock for how many seconds: '))
    method = "walletpassphrase"
...
ip_choice = input('What is the IP address of the wallet to control (leave blank for local machine): ')
if ip_choice == '':
    ip_choice = '127.0.0.1'    
